...
{	
	public static ISFSObject genCardDirtyInfo(CardBean card,int player,int receiver){
		...
		if(card.getDirtyFlagBit(CardBean.ATTACH_DIRTY_BIT))	
			cardInfo.putInt("attach", card.getAttachTo());
		...
		if(card.getDirtyFlagBit(CardBean.MP_DIRTY_BIT))	
			cardInfo.putInt("mp", card.getMp());
